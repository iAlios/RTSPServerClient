...
/**
 * RFC 3640. This packetizer must be fed with an InputStream containing ADTS
 * AAC. AAC will basically be rewrapped in an RTP stream and sent over the
 * network. This packetizer only implements the aac-hbr mode (High Bit-rate AAC)
 * and each packet only carry a single and complete AAC access unit.
 */
	...
	public void run() {
		...
		try {
			while (!Thread.interrupted()) {
				...
				// Parse adts header (ADTS packets start with a 7 or 9 byte long
				// header)
				fill(header, 2, 5);

				// The protection bit indicates whether or not the header
				// contains the two extra bytes
				...
				// The number of RTP packets that will be sent for this ADTS
				// frame
