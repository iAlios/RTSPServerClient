...
public class RTPSocket implements Runnable {
	...
	/**
	 * Returns an available buffer from the FIFO, it can then be modified. Call
	 * {@link #commitBuffer(int)} to send it over the network.
	 * 
	...
	/**
	 * Returns an approximation of the bitrate of the RTP stream in bit per
	 * seconde.
	 */
	...
	public void run() {
		...
		try {
			...
			while (mBufferCommitted.tryAcquire(4, TimeUnit.SECONDS)) {
				if (mOldTimestamp != 0) {
					// We use our knowledge of the clock rate of the stream and
					// the difference between two timestamps to
					...
					if ((mTimestamps[mBufferOut] - mOldTimestamp) > 0) {
						...
						// We ensure that packets are sent at a constant and
						// suitable rate no matter how the RtpSocket is used.
						if (mCacheSize > 0)
							Thread.sleep(d);
				...
				mReport.update(mPackets[mBufferOut].getLength(), System.nanoTime(),
				        (mTimestamps[mBufferOut] / 100L) * (mClock / 1000L) / 10000L);
				mOldTimestamp = mTimestamps[mBufferOut];
				if (mCount == 0)
					Log.e(TAG, "C: " + mClock + " TS: " + SystemClock.elapsedRealtime());
				if (mCount++ > 30)
