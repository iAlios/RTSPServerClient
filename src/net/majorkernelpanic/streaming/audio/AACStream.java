 ...
 * Call {@link #setDestinationAddress(java.net.InetAddress)} & {@link #start()}
 * and that's it ! Call {@link #stop()} to stop the stream.
 */
public class AACStream extends AudioStream {
	 ...
	 * profile) needs to be stored once {@link #generateSessionDescription()} is
	 * called.
	 * 
	 * @param prefs The SharedPreferences that will be used to store the
	 *            sampling rate
	 */
	...
	protected void encodeWithMediaCodec() throws IOException {
		...
		final int bufferSize = AudioRecord.getMinBufferSize(mQuality.samplingRate,
		        AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT) * 2;

		((AACLATMPacketizer) mPacketizer).setSamplingRate(mQuality.samplingRate);

		mAudioRecord = new AudioRecord(MediaRecorder.AudioSource.MIC, mQuality.samplingRate,
	...
	public String generateSessionDescription() throws IllegalStateException, IOException {
		...
		if (mMode == MODE_MEDIARECORDER_API) {
			...
			// frame length
			// config: contains the object type + the sampling rate + the
			// channel number
	 ...
	 * the sampling rate really is On some phone indeed, no error will be
	 * reported if the sampling rate used differs from the one selected with
	...
	private void testADTS() throws IllegalStateException, IOException {
		...
		final String TESTFILE = Environment.getExternalStorageDirectory().getPath()
		        + "/spydroid-test.adts";
		...
		// 5 bits for the object type / 4 bits for the sampling rate / 4 bits
		// for the channel / padding
		mConfig = mProfile << 11 | mSamplingRateIndex << 7 | mChannel << 3;
		...
		if (mSettings != null) {
			...
			editor.putString("aac-" + mQuality.samplingRate, mActualSamplingRate + "," + mConfig
			        + "," + mChannel);
			editor.commit();
		}
